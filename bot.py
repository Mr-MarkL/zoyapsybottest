...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    uname_display = f"@{user.username}" if user.username else f"без username"
...
async def on_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Регистрируем любого не-админа при первом сообщении (даже без /start)
    user = update.effective_user
    if user and not is_admin(update):
        full_name = user.full_name or "без имени"
        new_user = is_new_visitor(user.id)
        upsert_visitor(user.id, user.username or "", full_name)
        if new_user:
            uname_display = f"@{user.username}" if user.username else "без username"
            msg = (
                "👋 Новый пользователь!\n\n"
                f"👤 {full_name}\n"
                f"📎 {uname_display}\n"
            ...
            await notify_admins(ctx.application, msg)
