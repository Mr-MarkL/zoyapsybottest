...
async def notify_admins(app, text):
    """Шлёт уведомление всем admin-пользователям у которых есть tg_id в БД."""
    c = db()
    for uname in ADMINS:
        row = c.execute("SELECT tg_id FROM visitors WHERE LOWER(username)=?", (uname.lower(),)).fetchone()
        if row:
            try:
                await app.bot.send_message(chat_id=row["tg_id"], text=text)
